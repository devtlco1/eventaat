...
> **Source of truth:** the complete route list, roles, and tables are in **[`docs/api-reference.md`](../../docs/api-reference.md)** and **[`docs/api-inventory.md`](../../docs/api-inventory.md)**. The app exposes **~57** HTTP operations across health, auth, users, me, and restaurants (including `PATCH` **`/me/profile`** and **`/me/password`**, table and event flows, in-app **notifications** under `me/notifications`, **staff** read-only `GET` **`/me/reservation-operations`**, and **detail GETs** under `me/…` reservations and `restaurants/…/reservations` / `…/event-reservations`).
