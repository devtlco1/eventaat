...
- **Index & counts:** [docs/api-inventory.md](docs/api-inventory.md) — single table of all routes, **59** endpoints, grouped by domain.
...
Milestones in place: monorepo foundation; **API** (Nest, health) + **Prisma/PostgreSQL**; **auth** (JWT, register/login) and **RBAC**; **users** (platform management); **restaurants** (CRUD, assignments for restaurant admins); **request-based table reservations** (optional table) with **availability** helper; **reservation status history** and lifecycle rules; **event nights (restaurant events)** with **platform approval**; **event reservations** (separate from table flow, **PENDING** until restaurant confirms, **capacity** on confirm); **admin** (Next.js, webpack) and **mobile** (Expo) apps; **My Reservations**; **operating settings**; **customer cancellation** (eligible requests); **restaurant profile** metadata and contacts; **global request validation** (DTO / `ValidationPipe`); documented local workflow; **CI** (see above). **Step 36** aligned **table vs event** reservation **API shapes** (explicit `type`, normalized detail GETs, **oldest→newest** `statusHistory`, and reasons where stored). **Step 37** added **database-backed in-app notifications** (`/me/notifications` for all roles) on key reservation **status changes** and **customer cancels** — no push, email, or SMS. **Step 38** made notifications **actionable** (mobile: opens **table vs event** `ReservationDetail`; admin: same **RESTAURANT/TABLE** vs **EVENT** lists with **query-based row highlight**). **Step 39** added **`GET /me/reservation-operations`** (restaurant and platform staff) and an admin **Reservation operations** page. **Step 40** restructured the admin app with a **sidebar** layout, **Reservations** section, and **global** booking list pages. **Step 40B** aligned the nav with a **hospitality-style** module layout (inspired by product flow, not code, from the legacy `old_eventaat` reference): **bookings** (restaurant, event, pending), **Event nights** (`/dashboard/events`) as a global list, **per-venue** flows unchanged, and **in-app** links to **`/dashboard/bookings/…`**. **Step 40C** cleaned the **admin shell** (no sidebar **section headings**, **logo-only** top bar, **Profile & account** + **light/dark** at the bottom, **`/dashboard/account`**) and added **`PATCH` `/me/profile`** and **`PATCH` `/me/password`**. **Step 40D** audits the main **bookings and event night** admin pages: **Pending work**, **Restaurant** vs **Event** bookings, **Event nights** (nights only), shared **UI helpers** (`AdminPageHeader` etc.), **deep links** to highlight rows, and **inline** capacity errors. **Step 40E** documents **local browser QA** for the admin (see [local-development.md](docs/local-development.md#admin-dashboard-browser-qa-step-40e)) and **troubleshooting** when `api:start` or admin dev fail (`node_modules` or port **3000**). **Smoke** checks in automation: API **`/health`**, **`POST /auth/login`** (401 for bad creds), admin **HTTP 200** for `/login` and `/dashboard` shell. **Full** interactive login, booking actions, and **notifications** deep links remain **manual** with your data. **Step 41** simplifies the **admin** **IA**: **fixed** **collapsible** **sidebar**, **header** **notification** **bell**, **client** table **pagination** (default 20), **Add** modals (restaurant, event night, staff table booking), **Users** split (**system** vs **mobile**), and **legacy** route **redirects** (no “pending work” in the menu). **Step 41B** is a **visual** pass: **shared** admin table/filter **tokens**, **dark** **mode** readability (tables, inputs, **pagination**), **header** **page titles**, **Notifications** list fixed (**API** errors are surfaced with **retry**; no fake “Failed to load” from non-`Error` throws), **enriched** **restaurant** names on loaded reservations, **table** **booking** actions **only** for valid **status** transitions, **no** **AppIcons** changes. **Step 42** unifies the **admin** **toolbar** (filters + **Add** + **Refresh** icon), **action** **icons** with **tooltips** and **aria-labels** (shared **NavIcons** SVG set, not AppIcons), **normalized** `NEXT_PUBLIC_API_URL` (avoids **Cannot GET** on the **Next** origin when the env is empty), **POST** **`/users`** for **Add member** (platform admin), and **CORS** for **127.0.0.1:3000** as well as **localhost:3000**. **HTTP** count **59** (includes **POST** **`/users`**). **API** usage: [apps/api/README.md#request-validation](apps/api/README.md#request-validation).
