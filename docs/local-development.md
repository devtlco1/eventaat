...
| **Admin / Next: `ERR_INVALID_PACKAGE_CONFIG` (e.g. `.../next/dist/compiled/commander/package.json`)** | Usually a **corrupted** or **incomplete** install (Node cannot parse that file). From the **repo root**: delete root `node_modules` and any workspace `node_modules` (e.g. `rm -rf node_modules` then for each of `apps/api`, `apps/admin`, `apps/mobile`, and `packages/*` remove `node_modules` if it exists), then `npm install` (or `npm ci`). A lighter first try: `rm -rf node_modules/next` at the root, then `npm install`. Then `unset NODE_ENV` and `npm run admin:dev` again. |
