...
**Deep linking (clients):** For the reservation-related types above, payloads include `restaurantId`, `reservationId` and/or `eventReservationId`, and `entityId` matching the primary table or event-reservation row. Mobile uses these to open **GET** `/me/reservations/:id` or `/me/event-reservations/:id`. The **admin** app (Step 40/40B) uses **bookings** list routes with row highlight: **table (restaurant)** → `/dashboard/bookings/restaurants?restaurantId=&reservationId=`; **event** → `/dashboard/bookings/events?restaurantId=&eventReservationId=&eventId=` (and `/dashboard/operations` etc. may redirect to `/dashboard/bookings/…` while preserving query). The **per-restaurant** list URLs under `/dashboard/restaurants/…/reservations` and `…/event-reservations` (same query params) remain valid. No new HTTP operations for this navigation.
