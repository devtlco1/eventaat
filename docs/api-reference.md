...
**Deep linking (clients):** For the reservation-related types above, payloads include `restaurantId`, `reservationId` and/or `eventReservationId`, and `entityId` matching the primary table or event-reservation row. Mobile uses these to open **GET** `/me/reservations/:id` or `/me/event-reservations/:id`. The **admin** app (Step 40+) uses the **global** list routes with row highlight: **table** → `/dashboard/reservations/tables?restaurantId=&reservationId=`; **event** → `/dashboard/reservations/events?restaurantId=&eventReservationId=&eventId=`. The older **per-restaurant** list URLs (same query params) remain valid. No new HTTP operations for this navigation.
