...
| **Admin dashboard** | Next.js: **light, business-style** **shell** (Step 40C). **Step 40D** clarifies **bookings** screens: **Pending work** = daily queue; **Restaurant bookings** and **Event bookings** = full lists (filters, highlight via query; capacity errors for event **CONFIRM**); **Event nights** = scheduled nights (platform approve/reject), not guest bookings. **no visible sidebar group labels**; top **branding** only; **main nav** + **bottom** **Profile & account** / **light/dark**; **`/dashboard/account`**. **In-app** **deep links** to booking lists; legacy **`/operations`** / **`/reservations/*`** may **redirect**. **Per-venue** pages unchanged. **Work queue** read model: **`GET /me/reservation-operations`**. **HTTP** count unchanged in 40D (**57** with **Step 40C** profile routes). |
