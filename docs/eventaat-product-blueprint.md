...
| **Admin dashboard** | Next.js: restaurants, tables, users, admin assignments, **per-restaurant events**, **per-restaurant table reservations** and a separate **per-restaurant event reservations** list with confirm/reject, plus a **Reservation operations** screen (`/dashboard/operations`) for restaurant and platform staff, backed by **`GET /me/reservation-operations`** (pending and recent work in one place, with **links** to the same per-restaurant list URLs as in-app notification deep links). |
