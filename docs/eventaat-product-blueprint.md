...
| **Admin dashboard** | Next.js: **light, business-style** **sidebar** (not a “devops” shell): **Dashboard** (counters, pending work), **Restaurants**, **Event nights** (global `GET` merge per restaurant), **Bookings** (Restaurant bookings, Event bookings, and **All pending & recent** = former operations queue) at `/dashboard/bookings/…`, **Notifications**, **Users** (platform only). **In-app** notification **deep links** use `…/bookings/restaurants?…` and `…/bookings/events?…` (and legacy `…/operations` / `…/reservations/…` may **redirect**). **Per-venue** pages unchanged. **Aggregated** queue: **`GET /me/reservation-operations`**. |
