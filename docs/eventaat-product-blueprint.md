...
| **Admin dashboard** | Next.js: **Step 42** unifies **toolbars** (filter row + **Add** + **Refresh** icons), **row** **actions** as **icons** (tooltips + ARIA) using shared **NavIcons** SVGs (**AppIcons** untouched), **Add member** via **POST** **`/users`**, and **base URL** normalization so `fetch` always targets the **API** host. Prior **41** / **41B** shell (sidebar, **dark** **mode**, **enriched** restaurant names, **status**-aware booking actions) remains. Menu: **Dashboard**, **Restaurants**, **Event nights**, **Restaurant (table) bookings**, **Notifications**, **Users** (platform only). **59** **HTTP** routes. Earlier **account** / **theme** / **browser QA** remain. |
