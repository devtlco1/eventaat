...
| **Admin dashboard** | Next.js: **light, business-style** **shell** (Step 40C). **Step 40D** clarifies **bookings** screens; **Step 40E** is **local browser QA** and ops notes (troubleshooting **API start** if `node_modules` is bad; **manual** login, sidebar, **notifications** deep links). **Pending work** = daily queue; **Restaurant** / **Event** bookings = full lists; **Event nights** = platform approval of the **night**, not the guest list. **57** **HTTP** routes. |
