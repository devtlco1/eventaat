...
| **Admin dashboard** | Next.js: **business** shell (**Step 41**): **fixed** **collapsible** **sidebar** (main content scrolls); menu shows **Dashboard**, **Restaurants**, **Event nights**, **Restaurant** **(table) bookings**, **Notifications**, **Users** (platform only). **Operations** / **Pending work** / old **reservation** entry URLs **redirect**; **table** and **event-night** **guest** flows stay **separate**. **Header** **bell** for notifications; **all** **lists** default to **paged** tables (**20** / client until server pagination). Staff can create **PENDING** **table** requests via **POST** **`/restaurants/:id/reservations/admin`**. **58** **HTTP** routes. Earlier steps (**40C–E**) **account** / **theme** / **browser QA** remain. |
