...
| **Admin dashboard** | Next.js: **light, business-style** **shell** (Step 40C): **no visible sidebar group labels**; top **branding** only; **main nav** (Dashboard, venues, event nights, pending work, restaurant/event bookings, notifications, Users for **PLATFORM_ADMIN** only); **bottom** **Profile & account** and **light/dark**; **`PATCH` `/me/profile`** / **`/me/password`** for self-service from **`/dashboard/account`**. **In-app** notification **deep links** use `…/bookings/restaurants?…` and `…/bookings/events?…` (legacy `…/operations` / `…/reservations/…` may **redirect**). **Per-venue** pages unchanged. **Work queue** read model: **`GET /me/reservation-operations`**. |
