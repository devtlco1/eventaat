...
| **Admin dashboard** | Next.js: **left sidebar** shell (desktop) with a compact **mobile** menu; main areas are **Overview**, a **Reservations** workspace (**Operations** at `/dashboard/operations` — all venues in scope, pending + 7d recent, optional **confirm/reject** on PENDING), **Table reservations** and **Event reservations** at `/dashboard/reservations/tables` and `…/events` (global merged lists, filters, actions), plus **Restaurants**, **Notifications**, and **Users** (platform only). The **per-restaurant** event and table lists under `/dashboard/restaurants/:id/...` are unchanged. In-app **notification** links from the API open the **global** list routes with the same `restaurantId` and row highlight query params. Staff **aggregated** queue: **`GET /me/reservation-operations`**. |
